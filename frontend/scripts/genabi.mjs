...
const CONTRACT_NAME = "GeoChainFHE";

// contracts root (frontend -> ../contracts)
const rel = "../contracts";
...
if (!fs.existsSync(dir)) {
  console.error(
    `${line}Unable to locate ${rel}. Expecting action/contracts at ${dir}${line}`
  );
  process.exit(1);
}
...
*/
export const GeoChainABI = ${JSON.stringify({ abi: deployLocalhost.abi }, null, 2)} as const;
\n`;
const tsAddresses = `
/*
  This file is auto-generated.
  Command: 'npm run genabi'
*/
export const GeoChainAddresses = { 
...
console.log(`Generated ${path.join(outdir, `GeoChainABI.ts`)}`);
console.log(`Generated ${path.join(outdir, `GeoChainAddresses.ts`)}`);

fs.writeFileSync(path.join(outdir, `GeoChainABI.ts`), tsCode, "utf-8");
fs.writeFileSync(
  path.join(outdir, `GeoChainAddresses.ts`),
  tsAddresses,
  "utf-8"
);
